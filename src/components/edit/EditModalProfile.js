...
import { updateAvatarUser } from "../../firebase/firebaseGET";
import { getStorage, ref, deleteObject } from "firebase/storage";

export default function EditModalProfile({modalOpen,setModalOpen,avatarSrc,openInput,userId,}) {
  const deleteAvatar = async () => {
    const storageRefQoyMaPlisos = storage.refFromURL(avatarSrc);
    const fullPath = storageRefQoyMaPlisos.fullPath;
          ...
          <div className="border-0 rounded-lg shadow-2xl relative p-10 flex flex-col w-full bg-white outline-none focus:outline-none">
            ...
            <div className="flex flex-col p-8 ">
              ...
              <div className="flex justify-center px-4 py-2 mt-2 border">
                <button className="font-bold" onClick={() => setModalOpen(false)}>Cancel</button>
              </div>
            </div>
          </div>
        </div>
